function [Actual_Sleep_Times_Pre, Actual_Sleep_Times_Post] = Find_Sleep_autoEMG(mousedir, length_for_sleep_sec, min_interinterval_sec,speedthresh,jerkthresh,plotit)
...
%after doing a lot of manual scoring, it kinda seems like either EMG is
%fine as long as its got signal
minimum_duration_in_samples = length_for_sleep_sec*emgfs;
minimum_inter_interval_period=min_interinterval_sec*emgfs;

%I'm just going to say that col 1 is time, and use col 2 as the data, this
%will favor the first EMG but I think thats fine
% figure
% subplot(2,1,1)
...
% 
% figure
% plot(filteredEMGs(:,1),filteredEMGs(:,EMG2use+1))
% ylim([0 500]);
...
% MAKE EMG THRESHOLD
% filteredEMGs(:,2)=filteredEMGs(:,2)-mean(filteredEMGs(:,2));
%to remove offset

% upper=prctile(filteredEMGs(:,2),95);
% trimIX=filteredEMGs(:,2)<upper;
m=mean(filteredEMGs(:,2));
s=std(filteredEMGs(:,2));
EMG_thresh_auto=m-(3*s);
...
%Interpolating jerk data to be same length as EMG
Interpjerk= interp1(jerk(:,1),jerk(:,5),filteredEMGs(:,1));
Jerk(:,2)=Interpjerk;
Jerk(:,1)=filteredEMGs(:,1);
...
[PRE_EMG_SLEEPIX, PRE_EMG_START_END_IX, PRE_EMG_NEW_START_END_IX] =sleep_intervals_Monroe(PRE_EMG,EMG_thresh_auto, minimum_duration_in_samples,minimum_inter_interval_period);
 SLEEP_EMG_PRE=PRE_EMG_NEW_START_END_IX;
[POST_EMG_SLEEPIX, POST_EMG_START_END_IX, POST_EMG_NEW_START_END_IX] =sleep_intervals_Monroe(POST_EMG,EMG_thresh_auto, minimum_duration_in_samples,minimum_inter_interval_period);
...
hline = refline([0 EMG_thresh_auto]);
hline.Color = 'r';
title('PRE EMG')
...
save('EMGthresh.mat','EMG_thresh_auto');
