...
[spindle_channels{1,6}, ~, spindle_channels{1,4}] = spindle_filter_wiegand(EEG_channels, 300, .01);
...
for iSpindle_channels = 1:Rows(spindle_channels)
    ...
    if ~isempty(restrict_times)
        ...
        for iSleep_times = 1:Rows(sleep_above_times)
            ...
            %% Thresholding
            lower_spin_thresh = (std_chan*1);
            upper_spin_thresh = (std_chan*2.75);
            ...
            for iSee = 1:Rows(spin_above_times)
                ...
                if length(putative_spindle) > 3*sr
                    continue
                end
