function MLi2_INTAN_Post_Process(plot_it)
...
%The LC version just doesn't eliminate a channel's info if it has a bad
%impedance- I wanted this to happen later so that we know its being read
%and are told that the impedance is high

fs_initial = IF.frequency_parameters.amplifier_sample_rate;
bit2uV= IF.bit_to_uvolt_conversion;
...
if isempty(dir('board-DIN-08.dat'))|| isempty(dir('board-DIN-09.dat')) ||isempty(dir('board-DIN-10.dat'))
    disp('Just FYI there are no DINs in here- your settings might be wonky')
    MLi2_INTAN_Post_Process_nodins
    return
end
...
for ielement=1:length(dets)
    if  strfind(dets{ielement},'Day')
        day=dets{ielement};
    elseif strfind(dets{ielement},'20Hour')
        day='Day20Hour';
    end
end
...
for iport=1:length(possibleports)
    ...
    if ~isempty(Auxmouse) %adding this because if the aux isn't saved because no mouse then this will be empty
        ...
        continue
    end %removig this if statement since CTT was redefined to have only valid ports
    ...
    mousefold=[mouse day];
    save(fullfile([pwd '/' mousefold '/' 'AUXs_downsamp_port' possibleports(iport)]),'AUXs','-v7.3')
    save(fullfile([pwd '/' mousefold '/' 'jerk_port' possibleports(iport)]),'jerk','-v7.3')
    fprintf('.%d',auxnum)
    %     end
